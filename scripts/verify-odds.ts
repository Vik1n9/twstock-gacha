...
async function main() {
  ...
  const poolArgIdx = process.argv.indexOf("--pool");
  const poolFilter = poolArgIdx !== -1 ? process.argv[poolArgIdx + 1] : null;

  const allPools = await db.select().from(pools).where(eq(pools.active, true));
  const targets = poolFilter
    ? allPools.filter((p) => p.poolId === poolFilter)
    : allPools;
  if (targets.length === 0) {
    throw new Error(
      poolFilter ? `找不到卡池 ${poolFilter}，請先 seed` : "無活躍卡池，請先 seed",
    );
  }

  let anyFail = false;
  for (const pool of targets) {
    const fail = await verifyPool(db, pool);
    if (fail) anyFail = true;
    console.log("");
  }
  console.log(anyFail ? "驗證失敗" : "機率驗證全部通過");
...
async function verifyPool(
  ...
  const [snapshot] = await db
    .select()
    .from(poolSnapshots)
    .where(and(eq(poolSnapshots.poolId, pool.poolId), eq(poolSnapshots.isOpen, true)))
    .orderBy(desc(poolSnapshots.snapshotDate))
    .limit(1);
  if (!snapshot) {
    console.log(`跳過 ${pool.poolName}：無開放快照`);
    return false;
  }
  ...
  // 全市場池固定 50/50（企劃書 9.1）；板塊池動態（9.2）
  const fixedUpRate = pool.poolType === "market" ? 0.5 : null;
  const state = buildPoolState(rows, fixedUpRate);
  const upRate =
    fixedUpRate ?? state.upStockCount / (state.upStockCount + state.downStockCount);
  console.log(
  ...
  // N 取 30 萬：C/R 桶容許 0.3% ≈ 4σ，避免統計雜訊誤報；
  // 種子混入 poolId，讓各池獨立抽樣不互相相關
  const N = 300_000;
  let seed = 20260906;
  for (const ch of pool.poolId) seed = (seed * 31 + ch.charCodeAt(0)) >>> 0;
  ...
  for (let i = 0; i < N; i++) {
    ...
    if (!drawn) throw new Error(`${pool.poolName} 方向空池異常`);
  ...
  const checkDir = (dir: "UP" | "DOWN") => {
    const total =
      finalByDir[dir].C + finalByDir[dir].R + finalByDir[dir].SR + finalByDir[dir].SSR;
    const expectedDirRate = dir === "UP" ? upRate : 1 - upRate;
    check(`${dir} 方向率`, total / N, expectedDirRate, 0.005);
    for (const rarity of RARITY_LIST) {
      const chain: Rarity[] = ["SSR", "SR", "R", "C"];
      if (!has(dir, rarity)) {
        // 缺桶：理論上不應抽出該稀有度
        check(`${dir} ${rarity}（應為 0）`, finalByDir[dir][rarity] / total, 0, 0.0001);
        continue;
      }
